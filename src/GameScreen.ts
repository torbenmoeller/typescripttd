module TypeScriptTD {
    ...
    export class GameScreen extends Phaser.State {
        ...
        //UI
        TowerButtons: Array<TowerButton> = Array<TowerButton>();
        entry: Phaser.Image;
        exit: Phaser.Image;
        ...
        PauseButton: Button;
        SpeedButton: Button;
        NextWaveButton: Button;

        txtCash: Phaser.Text;
        txtLives: Phaser.Text;
        txtPoints: Phaser.Text;
        txtWave: Phaser.Text;

        //Buildmode
        buildSprite: Phaser.Sprite;
        ...
        Grid: Grid = new Grid(19, 11);
        Spawner: Spawner;
        Towers: Array<TowerInstance> = Array<TowerInstance>();
        Enemies: Array<EnemyInstance> = Array<EnemyInstance>();
        Paths: PathPlanner = new PathPlanner();


        public CurrentWave: WaveData;
        private _waveIndex: number;
        TimeToWaveSpawn: number;
        CurrentPoints: number;

        MapId: number = 1;
        Map: MapData;

        towerIds = new Array("tower-gun", "tower-laser", "tower-missile", "tower-quake-wave", "tower-rocket");
        weaponIds = new Array("fx-small-round", "fx-laser", "fx-missile", "fx-quake-wave", "fx-rocket");
        creepIds = new Array("flyer-fast", "flyer-slow", "land-basic", "land-fast", "land-slow");

        preload() {
            var _this = this;
            this.game.stage.disableVisibilityChange = true;
            this.MapId = LevelSelectScreen.mapChosen;
        ...
        create() {
            ...
            this.Grid = new Grid(19, 11);
            this.buildMode = false;
            this.Speed = 1;
            this.CurrentPoints = 0;
            this.Towers = Array<TowerInstance>();
            this.Enemies = Array<EnemyInstance>();
            this.TowerButtons = Array<TowerButton>();
            this.buildMode = false;
            ...
            //Load Map
            this.Map = MapData.fromJson(this.cache.getText("map").toString());
            //Set First Wave
            ...
            //Add Entry And Exit
            var start: Vector2 = this.Grid.getCellCenter(this.Map.StartX, this.Map.StartY);
            this.entry = this.game.add.sprite(start.x - 55, start.y, "enter");
            this.entry.anchor.set(0.5, 0.5);
            var end: Vector2 = this.Grid.getCellCenter(this.Map.EndX, this.Map.EndY);
            ...
            //Add TowerButtons
            this.TowerButtons[0] = new TowerButton(this.game, new Vector2(535, 445), this.towerIds[0]);
            this.TowerButtons[1] = new TowerButton(this.game, new Vector2(590, 445), this.towerIds[1]);
            this.TowerButtons[2] = new TowerButton(this.game, new Vector2(645, 445), this.towerIds[2]);
            this.TowerButtons[3] = new TowerButton(this.game, new Vector2(700, 445), this.towerIds[3]);
            this.TowerButtons[4] = new TowerButton(this.game, new Vector2(755, 445), this.towerIds[4]);
            this.DisableButton = new Button(this.game, new Vector2(470, 445), "Disable");
            this.PauseButton = new Button(this.game, new Vector2(10, 437), "Play");

            this.SpeedButton = new Button(this.game, new Vector2(70, 437), "X1");
            this.NextWaveButton = new Button(this.game, new Vector2(120, 437), "NextWave");
            ...
            this.UpgradeButton = new Button(this.game, new Vector2(0, 0), "Upgrade")
            this.UpgradeButton.Sprite.visible = false;
            this.SellButton = new Button(this.game, new Vector2(0, 0), "Sell")
        ...
        onTowerButtonClick(towerdId: string) {
            //DestroeBuildSprite
        ...
        onGridClick() {
            var cellClicked: Cell = this.Grid.getCellAtPixel(new Vector2(this.game.input.mousePointer.x, this.game.input.mousePointer.y));
        ...
        hideUpdateButtons() {
            if (this.UpgradeButton) {
                if (this.UpgradeButton.Sprite.visible == true) {
                    ...
                    this.UpgradeButton.Position = new Vector2(this.game.width, this.game.height);
                }
            }
            if (this.SellButton) {
                if (this.SellButton.Sprite.visible == true) {
                    ...
                    this.SellButton.Position = new Vector2(this.game.width, this.game.height);
                }
            }
        }

        showUpdateButtons(v: Vector2) {
            this.UpgradeButton = new Button(this.game, v.subtract(new Vector2(57, 17)), "Upgrade")
            this.SellButton = new Button(this.game, v.add(new Vector2(23, -17)), "Sell")
        }

        upgradeTower() {
            var towerInstance = this.CellClicked.towerInstance;
            var nextlevel: number = towerInstance.Data.Level + 1;
            if (nextlevel >= 3)
                return;
            var towerId: string = towerInstance.Data.Id;
            var towerData = TowerData.fromJson(this.cache.getText(towerId + "data" + "-" + nextlevel).toString());
        ...
        BuildTower(towerName: string, cellX: number, cellY: number) {
            ...
            if (c.buildable) {
                ...
                var allowed: boolean = this.Paths.PathExist(startCell, endCell);
                c.buildable = true;
                if (!allowed)
                    return;

                //Fill instance
                var towerData = TowerData.fromJson(this.cache.getText(towerName + "data" + "-" + "0").toString());
                //Build Tower if enough cash
                if (towerData.Cost <= this.RemainingCash) {

                    //Create Instance
                    var t: TowerInstance = new TowerInstance();
                    t.Data = towerData;
                    t.init();
                    this.setTxtCash(this.RemainingCash - towerData.Cost);
        ...
        update() {
            ...
            var timeElapsed: number = this.game.time.elapsed * this.Speed / 1000;
